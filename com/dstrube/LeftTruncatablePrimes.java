...
public class LeftTruncatablePrimes{
	...
	public static void main(String[] args){
		...
		final BigInteger largestLeft = new BigInteger("357686312646216567629137"); //357_686_312_646_216_567_629_137 =~ 357 sextillion
...
Exception in thread "main" java.lang.OutOfMemoryError: Java heap space
	...
	at com.dstrube.LeftTruncatablePrimes.lambda$readFile$0(LeftTruncatablePrimes.java:116)
		...
		*/
		...
		final BigInteger pseudo1 = new BigInteger("1195068768795265792518361315725116351898245581");
		is = pseudo1.isProbablePrime(1);
		//System.out.println("Is " + pseudo1 + " probably prime? " + is); //false
		
		final BigInteger pseudo2 = new BigInteger("");
		...
		//Would it incorrectly tell us there is a larger LeftTruncatablePrimes than 357686312646216567629137?
		//This cannot be determined with current methods given the limited range of Sieve.java
		
		//At 5:24, the above video says this is the largest right truncatable prime:
		final BigInteger largestRight = new BigInteger("73939133");
		//Prove it...
		
		//Maybe do this someday:
		final BigInteger largestLeftRight = new BigInteger("739397");
		
		//Not todo: prove that any-deleteable primes are infinite :-p
		
		final List<Integer> primesList = new ArrayList<>(primes);
		final int lastInList = primesList.get(primes.size() - 1);
		...
		*/
		...
		System.out.println("Looking for right truncatable primes in the primes list...");
		for (int i = primesList.size() - 1; i > 0; i--){
			if (isRightTruncatablePrime(primesList.get(i))){
				System.out.println("Largest right truncatable found: " + primesList.get(i)); //73_939_133 - Proven!
				break;
			}
		}
	...
	private static boolean readFile(final Path path) {
	...
	private static boolean isLeftTruncatablePrime(final int candidate){
		...
		final int newNumber = Integer.parseInt(numString);
		return isLeftTruncatablePrime(newNumber);
	}
	
	private static boolean isRightTruncatablePrime(final int candidate){
		//If this is not in the set of primes, return false
		if (!primes.contains(candidate)){
			return false;
		}
		...
		//Trim off last digit
		final int newNumber = candidate / 10;
		return isRightTruncatablePrime(newNumber);
